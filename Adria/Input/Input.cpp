...
namespace adria
{
	...
	void Input::NewFrame()
	{
		...
		if (Window::IsActive())
		{
			...
			{
				// Keys
				keys[KeyCode::MouseLeft] = (::GetKeyState(VK_LBUTTON) & 0x8000) != 0; // Left button pressed
				keys[KeyCode::MouseMiddle] = (::GetKeyState(VK_MBUTTON) & 0x8000) != 0; // Middle button pressed
				keys[KeyCode::MouseRight] = (::GetKeyState(VK_RBUTTON) & 0x8000) != 0; // Right button pressed
			}
