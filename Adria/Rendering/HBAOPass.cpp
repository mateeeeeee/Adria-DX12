...
namespace adria
{
	...
	void HBAOPass::AddPass(RenderGraph& rendergraph)
	{
		...
		rendergraph.AddPass<HBAOPassData>("HBAO Pass",
			...
			[&](HBAOPassData const& data, RenderGraphContext& context, GraphicsDevice* gfx, CommandList* cmd_list)
			{
				...
				cmd_list->SetPipelineState(RootSigPSOManager::GetPipelineState(EPipelineState::HBAO));
				...
				D3D12_CPU_DESCRIPTOR_HANDLE src_ranges[] = { context.GetReadOnlyTexture(data.gbuffer_normal_srv), context.GetReadOnlyTexture(data.depth_stencil_srv), hbao_random_texture->GetSubresource_SRV() };
	...
	void HBAOPass::OnSceneInitialized(GraphicsDevice* gfx)
	{
		...
		hbao_random_texture->CreateSubresource_SRV();
