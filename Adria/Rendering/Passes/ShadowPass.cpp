...
namespace adria
{
	...
	ShadowPassData const& ShadowPass::AddPass(RenderGraph& rg, Light const& light)
	{
		...
		case ELightType::Directional:
			if (light.use_cascades) return ShadowMapPass_DirectionalCascades(rg, light);
			else return ShadowMapPass_Directional(rg, light);
		case ELightType::Spot:
			return ShadowMapPass_Spot(rg, light);
		case ELightType::Point:
			return ShadowMapPass_Point(rg, light);
		default:
			ADRIA_ASSERT(false);
		}
		return {};
	...
	ShadowPassData const& ShadowPass::ShadowMapPass_Directional(RenderGraph& rg, Light const& light, std::optional<DirectX::BoundingSphere> const& scene_bounding_sphere)
	{
		...
		return rg.AddPass<ShadowPassData>("Shadow Map Directional Pass",
			[=](ShadowPassData& data, RenderGraphBuilder& builder)
			{
				...
				RGTextureRef depth_stencil = builder.CreateTexture("Shadow Depth Map", depth_desc);
				...
				data.depth_map_dsv = builder.CreateDSV(depth_stencil, dsv_desc);
				data.depth_map_srv = builder.CreateSRV(depth_stencil, srv_desc);
			...
			[=](ShadowPassData const& data, RenderGraphResources& resources, GraphicsDevice* gfx, CommandList* cmd_list)
			{
				...
				DynamicAllocation shadow_allocation = upload_buffer->Allocate(GetCBufferSize<ShadowCBuffer>(), D3D12_CONSTANT_BUFFER_DATA_PLACEMENT_ALIGNMENT);
