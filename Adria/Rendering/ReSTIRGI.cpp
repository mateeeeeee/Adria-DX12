	...
	void ReSTIRGI::CreateBuffers()
	{
			...
			temporal_reservoir_buffers[0].reservoir->GetHeight() != height)
		{
			for (uint32 i = 0; i < ARRAYSIZE(temporal_reservoir_buffers); ++i)
			{
				...
				sample_radiance_desc.initial_state = GfxBarrierFlag_ComputeUAV;
				...
				ray_direction_desc.initial_state = GfxBarrierFlag_ComputeUAV;
				...
				reservoir_desc.initial_state = GfxBarrierFlag_ComputeUAV;
