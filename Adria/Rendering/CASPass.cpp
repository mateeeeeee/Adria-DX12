...
namespace adria
{
	CASPass::CASPass(GfxDevice* gfx, uint32 w, uint32 h) : gfx(gfx), width(w), height(h)
	{
		...
		uint64 const scratch_buffer_size = ffxGetScratchMemorySizeDX12(FFX_CAS_CONTEXT_COUNT);
		void* scratch_buffer = malloc(scratch_buffer_size);
		FfxErrorCode error_code = ffxGetInterfaceDX12(&cas_context_desc.backendInterface, gfx->GetDevice(), scratch_buffer, scratch_buffer_size, FFX_CAS_CONTEXT_COUNT);
	...
	{
		DestroyContext();
		if (cas_context_desc.backendInterface.scratchBuffer != nullptr)
		{
			free(cas_context_desc.backendInterface.scratchBuffer);
		}
	}

	RGResourceName CASPass::AddPass(RenderGraph& rg, RGResourceName input)
	{
		struct CASPassData
		{
	...
	void CASPass::OnResize(uint32 w, uint32 h)
	{
		width = w, height = h;
		DestroyContext();
		CreateContext();
